...
bool Mesh::Collapse() {
  ...
  std::list<Triangle*> needAdjusting;
  ...
  for(std::list<Triangle*>::iterator iter=needAdjusting.begin();iter!=needAdjusting.end();iter++){
  	for(int i=0;i<3;i++){
  		if((*iter)->getVertex(i)==NULL){
  			(*iter)->setVertex(i,a);
  			break;
  		}
  	}
  }
  		////I NEED TO BE DOING SOME ADD EDGE ACTION RIGHT ABOUT HERE
...
void Mesh::RemoveEdge(Vertex *a_, Vertex *b_) {
  ...
  if (itr != edges.end()) {
  	...
  	edges_pq.remove(new Edge(a, b, this));
...
/*
    	PriorityQueue<Edge*>::iterator iter=edges_pq.begin();
    	while(iter!=edges_pq.end()){
    		if(iter->isLegal()){
    			answer=*iter;
    			break;
    		}
