...
double getAngle(Star & center, Star & outer1, Star & outer2){
	std::cout<<"("<<center.x()<<", "<<center.y()<<") ("<<outer1.x()<<", "<<outer1.y()<<") ("<<outer2.x()<<", "<<outer2.y()<<")\n";
	double angle;
	...
	for(std::list<Constellation>::iterator constellationsIter=constellations.begin(); constellationsIter!=constellations.end(); constellationsIter++){
		...
		if(foundFirstThree==true){
			...
			if(found!=false){
				...
				std::cout<<"FOUND CONSTELLATION "<<constellationsIter->getName()<<"\n";
				ostr<<"constellation   "<<constellationsIter->getName()<<"\n";
				...
				std::list<std::pair<std::string,std::string>> lines=constellationsIter->getLines();
				ostr<<std::setw(1);
