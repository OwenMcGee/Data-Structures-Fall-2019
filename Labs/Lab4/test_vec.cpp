...
int main() {
  ...
  Vec<int> matchTest;
  for(int i=0;i<15;i++){
    matchTest.push_back(i%5);
  }
  std::cout<<"\nContents of match removal testing vector:";
  for(unsigned int i=0;i<matchTest.size();i++){
    std::cout<<" "<<matchTest[i];
  }
  std::cout<<"\nNumber of elements removed: "<<remove_matching_elements(matchTest, 3)<<"\n";
  std::cout<<"New Contents of match removal testing vector:";
  for(unsigned int i=0;i<matchTest.size();i++){
    std::cout<<" "<<matchTest[i];
  }
  std::cout<<"\n";

  std::cout<<"\nNumber of elements removed: "<<remove_matching_elements(matchTest, 9)<<"\n";
  std::cout<<"New Contents of match removal testing vector:";
  for(unsigned int i=0;i<matchTest.size();i++){
    std::cout<<" "<<matchTest[i];
  }
