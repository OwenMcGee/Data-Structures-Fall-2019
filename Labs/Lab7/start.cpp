...
void read_grid(istream& istr, vector<vector<bool> > & blocked_grid,
	       int & start_x, int & start_y, vector<vector<bool>> & pathTaken) {
...
void print_grid(vector<vector<bool> > const& blocked_grid, int start_x, int start_y, vector<vector<bool>> pathTaken) {
...
int recursiveFind(int x, int y, vector<vector<bool>> & blocked_grid, vector<vector<bool>> pathTaken, vector<vector<bool>> & pathTakenFinal, vector<Point> points, vector<Point> & pointsFinal){
  ...
  pathTaken[x][y]=true;
  if((x==0)&&(y==0)){
    pathTakenFinal=pathTaken;
    pointsFinal=points;
    return true;
  }
  if(!recursiveFind(x-1,y,blocked_grid,pathTaken,pathTakenFinal,points,pointsFinal)){
    return recursiveFind(x,y-1,blocked_grid,pathTaken,pathTakenFinal,points,pointsFinal);
  }
  return true;
}
...
int main(int argc, char* argv[]) {
  ...
  vector<Point> points;
  vector<vector<bool>> pathTaken;
  vector<vector<bool> > blocked_grid;
  int start_x, start_y;
  read_grid(istr, blocked_grid, start_x, start_y, pathTaken);
  print_grid(blocked_grid, start_x, start_y, pathTaken);

  // Start here with your code...

  recursiveFind(start_x,start_y,blocked_grid, pathTaken, pathTaken, points, points);
  ...
  if(points.size()!=0)
    ...
    std::cout<<"There are no possible paths\n\n";
